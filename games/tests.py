...
from games.models import GameCategory, Player


class GameCategoryTest(APITestCase):
    ...
    def test_create_and_retrieve_game_category(self):
        ...
        print('PK {}'.format(GameCategory.objects.get().pk))
...
class PlayerTests(APITestCase):
    ...
    def test_create_duplicated_player(self):
        ...
        self.assertEqual(response1.status_code, status.HTTP_201_CREATED)
        response2 = self.create_player(new_player_name, new_player_name)
        self.assertEqual(response2.status_code, status.HTTP_400_BAD_REQUEST)
